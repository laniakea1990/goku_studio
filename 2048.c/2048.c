...
#include <time.h>
#include <signal.h>
#include <stdint.h>   //C标准函数库中的头文件，定义了具有特定位宽的整型，以及对应的宏
#define SIZE 6
uint32_t score=0;
...
int main(int argc, char *argv[]) {
	uint16_t board[SIZE][SIZE];   //设置游戏面板大小
	char c;
	bool success;

	if (argc == 2 && strcmp(argv[1],"test")==0) {  //测试程序的判断，'/2048 test'
		return test();
	}
	...
	signal(SIGINT, signal_callback_handler);   //当按下Ctrl+C后的信号处理函数

	memset(board,0,sizeof(board));  //初始化数组board，数组元素置0
	addRandom(board);   //随机的在board中选择两个方格，使其数字为2
	addRandom(board);
	drawBoard(board);   //每次移动一步后，对整个游戏面板进行重新布局（包括方格颜色）
